...
// Open an external link (wa.me, tel:, etc). Plain `window.open`/`<a target="_blank">`
// don't reliably hand off to other apps inside the Capacitor WebView — use the native
// Browser plugin there, which triggers Android's normal app-chooser/intent handling.
async function pbOpenExternal(url) {
...
// Share a captured canvas as a PNG. Inside the Capacitor Android app, `navigator.share`
// and blob-URL downloads either don't exist or silently do nothing in the WebView (no
// download manager to catch them) — so use Capacitor's native Share/Filesystem plugins
// there instead. In a normal browser (or the plain web PWA), keep the existing behavior.
async function pbShareCanvasAsPng(canvas, filename, shareTitle) {
  if (window.Capacitor?.isNativePlatform?.()) {
    ...
    await Share.share({ title: shareTitle, url: written.uri });
    return;
  }
...
// ── Customer link ──────────────────────────────────────────────────
// Customer-facing links must always point at the real public site — inside the
// Capacitor app, window.location.origin is a local app scheme, not something a
// customer can open. A short "r<rowNumber>" code (instead of the old long
// compressed-JSON blob) also makes the link look trustworthy enough to tap.
const PB_PUBLIC_SITE = 'https://www.thepainterboys.com';
function buildCustomerUrl(row) {
  return `${PB_PUBLIC_SITE}/job/r${row.__row}`;
}
...
function PBThankYouModal({ name, phone, customerUrl, onClose }) {
  ...
  async function shareCard() {
    ...
    try {
      ...
      await pbShareCanvasAsPng(canvas, 'thankyou-thepainterboys.png', 'The Painter Boys');
    } catch {}
    setCapturing(false);
  }
  ...
  const shareCustomer = () => {
    ...
    pbOpenExternal(phone ? waUrl : `https://wa.me/?text=${encodeURIComponent(msg)}`);
  };
      ...
      <div className="pr-wrap" onClick={e => e.stopPropagation()}>
        <div className="ty-card" ref={cardRef}>
          ...
          <div className="ty-body">
            ...
            <div className="ty-dear">Dear {(name && name !== '—') ? name : 'Customer'},</div>
...
function PBReceiptModal({ name, phone, society, address, fieldName, td, allTokenData = {}, totalJobAmount = 0, onClose }) {
  ...
  async function shareAsImage() {
    ...
    try {
      ...
      await pbShareCanvasAsPng(canvas, 'receipt-thepainterboys.png', 'Payment Summary — The Painter Boys');
    } catch {}
    setCapturing(false);
  }
...
function DetailSheet({ row, headers, rows, onClose, onSave, onDelete, saving, scriptUrl, onNeedScript, showToast }) {
  ...
  const saveCustomerUrlOnce = () => {
    ...
    const url = buildCustomerUrl(row);
  ...
  return (
                    ...
                    <>
                      ...
                      <div className="pb-share-customer-btns">
                        <a className="pb-share-wa" href={waUrl} target="_blank" rel="noopener noreferrer"
                          onClick={e => {
                            saveCustomerUrlOnce();
                            if (window.Capacitor?.isNativePlatform?.()) { e.preventDefault(); pbOpenExternal(waUrl); }
                          }}>
                          💬 Send on WhatsApp
                        </a>
    ...
    {showThankYou && (
      <PBThankYouModal
        ...
        customerUrl={buildCustomerUrl(row)}
      ...
      <div className="pb-card-top">
        ...
        <div className="pb-card-top-right">
          ...
          <a className="pb-wa-btn" href={waHref} target="_blank" rel="noopener noreferrer"
            onClick={e => {
              e.stopPropagation();
              if (window.Capacitor?.isNativePlatform?.()) { e.preventDefault(); pbOpenExternal(waHref); }
