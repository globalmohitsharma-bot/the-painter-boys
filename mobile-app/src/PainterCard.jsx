...
export default function PainterCard() {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      // Inside the Capacitor Android app, navigator.share/blob-URL downloads don't
      // work in the WebView — use the native Share/Filesystem plugins instead.
      if (window.Capacitor?.isNativePlatform?.()) {
        const dataUrl = canvas.toDataURL('image/png');
        const base64  = dataUrl.split(',')[1];
        const { Filesystem, Directory } = await import('@capacitor/filesystem');
        const { Share } = await import('@capacitor/share');
        const written = await Filesystem.writeFile({ path: `${name}_ThePainterBoys.png`, data: base64, directory: Directory.Cache });
        await Share.share({ title: `${name} — The Painter Boys`, text: waShareText, url: written.uri });
      ...
      canvas.toBlob(async (blob) => {
        ...
        if (navigator.share && navigator.canShare?.({ files: [file] })) {
          ...
          link.click();
          setTimeout(() => pbOpenExternal(`https://wa.me/?text=${encodeURIComponent(waShareText)}`), 800);
          setShared(true); setTimeout(() => setShared(false), 3000);
        }
        setSharing(false);
      }, 'image/png', 0.95);
      ...
      setSharing(false);
      pbOpenExternal(`https://wa.me/?text=${encodeURIComponent(waShareText)}`);
    }
  };
      ...
      <div className="pc-actions">
        <a className="pc-action pc-action-wa"
          ...
          onClick={e => {
            if (window.Capacitor?.isNativePlatform?.()) {
              e.preventDefault();
              pbOpenExternal(`https://wa.me/${CORPORATE.wa}?text=${encodeURIComponent('Hello, I want to know more about your painting services.')}`);
            }
          }}>
