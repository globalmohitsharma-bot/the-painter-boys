...
| SMTP Health Check (Utility → Settings) | "Send Test Email" sends a one-line email to the signed-in admin's own address via `POST /api/auth/test-email`, confirming SMTP works without triggering a real customer-facing email. Verified locally (returns `{"sentTo": "..."}`, matches the caller's JWT email claim) — SMTP itself is only configured on prod, so the real "did an email land in the inbox" check has to happen there, by an admin actually clicking the button | 2026-08-30 |
